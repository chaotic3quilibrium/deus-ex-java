...
/**
 * A validation wrapper restricting an {@code int} to a value greater than {@code 0}.
 * <p>
...
public record PosInt(int value) implements Comparable<PosInt> {
   ...
   * <ul>
   * <li>{@code value} must greater than {@code 0}</li>
   * </ul>
   *
   * @param value an {@code int} value greater than {@code 0}
  ...
  public static Optional<ParametersValidationException> invalidate(
    ...
    if (value < 1) {
      return Optional.of(new ParametersValidationException(
          "PosInt invalid parameter(s)",
          "value [%d] must be greater than 0".formatted(value)));
    }
   ...
   * instance from the call to the {@link #invalidate(int)} method.
   *
   * @param value an {@code int} value greater than {@code 0}
