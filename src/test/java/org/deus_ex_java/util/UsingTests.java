...
public class UsingTests {
  ...
  public void testSingleResource() {
    ...
    var runtimeExceptionOrIntegerA = Using.apply(
        () -> new StringReader("x"),
        stringReader -> FunctionsPrimitivesOps.wrapCheckedException(() -> stringReader.read(arrayCharsX100)).getAsInt()
    );
    ...
    var wrappedCheckedExceptionOrIntegerB = Using.apply(
        () -> new StringReader("x"),
        stringReader -> FunctionsPrimitivesOps.wrapCheckedException(() -> {
          stringReader.close();
  ...
  public void testTwoResources() {
    ...
    closeLog.clear();
    var valUnsafe = Using.applyUnsafe(
        () -> new MockResource("A", closeLog),
        () -> new MockResource("B", closeLog),
        tuple -> tuple._1().name + tuple._2().name
    );
    ...
    closeLog.clear();
    var resNested = Using.applyNested(
        () -> new MockResource("A", closeLog),
    ...
    closeLog.clear();
    var valNestedUnsafe = Using.applyNestedUnsafe(
        () -> new MockResource("A", closeLog),
        a -> new MockResource("B", closeLog),
        tuple -> tuple._1().name + tuple._2().name
    );
  ...
  public void testFatalThrowablePropagation() {
    assertThrows(
        ...
        () -> Using.apply(
            ...
            r -> {
              sneakyThrow(new InterruptedException("interrupted!"));
              return "ok";
            }
        )
    );
